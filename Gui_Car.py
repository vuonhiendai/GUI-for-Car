...
class Index():
    ...
    def __init__(self):
        ...
        self.createWides()
        self.blue = serial.Serial('COM3', baudrate='9600')
    ...
    def createWides(self):
        ...
        def start_click():
            ...
            self.blue.open()
            self.blue.write(b'start')

        def stop_click():
            Car_Status.configure(text = "Car status: \n Car is stopping!")
            self.blue.write(b'stop')
            self.blue.close()

        def up_click():
            Car_Status.configure(text = "Car status: \n Car goes straigth!")
            self.blue.write(b'Up')

        def left_click():
            Car_Status.configure(text = "Car status: \n Car turns left!")
            self.blue.write(b'left')

        def right_click():
            Car_Status.configure(text = "Car status: \n Car turns right!")
            self.blue.write(b'right')

        def down_click():
            Car_Status.configure(text = "Car status: \n Car turn back!")
            self.blue.write(b'down')    
        ...
        Right_Button = ttk.Button(control_lab, text="Right", command = right_click)
